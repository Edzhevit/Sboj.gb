...
public class JobApplicationRepositoryImpl implements JobApplicationRepository {
    ...
    public List<JobApplication> findAll() {
        this.entityManager.getTransaction().begin();
        List<JobApplication> applications = this.entityManager.createQuery("SELECT j FROM JobApplication AS j", JobApplication.class)
                .getResultList();
        this.entityManager.getTransaction().commit();
        return applications;
    }
    ...
    public JobApplication findById(String id) {
        ...
        return this.entityManager.createQuery("SELECT j FROM JobApplication AS j WHERE j.id = :id", JobApplication.class)
                .setParameter("id", id)
                .getSingleResult();
    ...
    public void delete(String id) {
        this.entityManager.getTransaction().begin();
        this.entityManager.createQuery("DELETE FROM JobApplication AS j WHERE j.id = :id")
                .setParameter("id", id)
                .executeUpdate();
        this.entityManager.getTransaction().commit();
    }
